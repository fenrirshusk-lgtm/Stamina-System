...
# ----- Helper to calculate individual costs -----
...
if mobility_inactive:
    ...
    base_cost /= 2
    # Rounding mobility: round up on .25 or .75 only
    if base_cost % 1 in [0.25, 0.75]:
        base_cost = math.ceil(base_cost * 2) / 2
    ep_mobility = max(base_cost, 1) if base_cost > 0 else 0

# ----- Total EP Cost Before Final Min -----
raw_cost = ep_power1 + ep_power2 + ep_range + ep_mobility + buff_debuff_cost + extra_costs

# Smart rounding: round .25/.75 up to nearest .5 or whole
if raw_cost % 1 in [0.25, 0.75]:
    total_cost = math.ceil(raw_cost * 2) / 2
else:
    total_cost = round(raw_cost)

# Enforce final minimum of 1 (only if > 0)
if total_cost > 0:
...
st.write(f"**Total EP Cost (after rounding, min 1 last)**: {total_cost}")
