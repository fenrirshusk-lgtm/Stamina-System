...
# ----- Apply rounding BEFORE minimum enforcement -----
ep_components = [ep_power1, ep_power2, ep_range, ep_mobility]
buff_debuff_cost = get_buff_debuff(buff_debuff)
control_reduction = get_control_reduction(control, control_inactive)

# Total cost pre-round
raw_cost = sum(ep_components) + buff_debuff_cost + extra_costs - control_reduction
...
# Enforce minimum cost of 1 after rounding
if total_cost > 0:
...
st.write(f"**Total EP Cost (rounded, then min 1 applied)**: {total_cost}")
